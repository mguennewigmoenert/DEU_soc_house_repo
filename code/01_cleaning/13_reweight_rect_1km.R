...
# set target ID for TDW file
int_tdw_lor_2021_rect_1km <- 
  int_tdw_lor_2021_rect_1km |>
  rename(target_id = PLR_ID)

names(int_w_lor_2021_rect_1km)
names(int_tdw_lor_2021_rect_1km)
names(panelSUF)

# ---- Join relevant variables --------------------------------------------
vars <- panelSUF |> dplyr::select(
  r1_id, year,
...
#   TRUE  -> use Target-Density Weights (TDW) from int_tdw_lor_2021_rect_1km
#   FALSE -> use simple area weights from int_w_lor_2021_rect_1km
# --------------------------------------------------------------------------
USE_TDW <- TRUE

if (USE_TDW) {
  
  # TDW crosswalk: idm (grid id), target_id (LOR), year, weight_tdw
  # 1) rename idm -> source_id
  # 2) join area weights "weights" from int_w_lor_2021_rect_1km
  # 3) join microm vars from panelSUF (vars)
  crosswalk <- int_tdw_lor_2021_rect_1km |>
    rename(source_id = idm) |>
    left_join(
      int_w_lor_2021_rect_1km |>
        dplyr::select(source_id, target_id, weights),
      by = c("source_id", "target_id")
    ) |>
    left_join(
      vars,
      by = c("source_id" = "r1_id", "year" = "year")
    )
  
} else {
  
  # Pure area-weight crosswalk: no TDW, only geometry-based weights
  crosswalk <- int_w_lor_2021_rect_1km |>
    # create TDW column equal to area weights (so downstream code still works)
    mutate(weight_tdw = weights) |>
    left_join(
      vars,
      by = c("source_id" = "r1_id"),
...
#   - weights    = area share A_st / A_s
#   - weight_tdw = TDW (or = weights if USE_TDW == FALSE)
# --------------------------------------------------------------------------
...
#
# ---- Aggregate absolute totals using weights -----------------------------
# Here we keep your distinction:
#   - TDW (weight_tdw) for people/HH-related quantities
#   - area weights (weights) for some stock-type variables
absolute_totals <- crosswalk |>
  ...
  summarise(
    ...
    # Male population
    r1_eag_m00bis03    = sum(r1_eag_m00bis03 * weight_tdw, na.rm = TRUE),
    r1_eag_m03bis06    = sum(r1_eag_m03bis06 * weight_tdw, na.rm = TRUE),
    r1_eag_m06bis10    = sum(r1_eag_m06bis10 * weight_tdw, na.rm = TRUE),
    r1_eag_m10bis15    = sum(r1_eag_m10bis15 * weight_tdw, na.rm = TRUE),
    r1_eag_m15bis18    = sum(r1_eag_m15bis18 * weight_tdw, na.rm = TRUE),
    r1_eag_m18bis20    = sum(r1_eag_m18bis20 * weight_tdw, na.rm = TRUE),
    r1_eag_m20bis25    = sum(r1_eag_m20bis25 * weight_tdw, na.rm = TRUE),
    r1_eag_m25bis30    = sum(r1_eag_m25bis30 * weight_tdw, na.rm = TRUE),
    r1_eag_m30bis35    = sum(r1_eag_m30bis35 * weight_tdw, na.rm = TRUE),
    r1_eag_m35bis40    = sum(r1_eag_m35bis40 * weight_tdw, na.rm = TRUE),
    r1_eag_m40bis45    = sum(r1_eag_m40bis45 * weight_tdw, na.rm = TRUE),
    r1_eag_m45bis50    = sum(r1_eag_m45bis50 * weight_tdw, na.rm = TRUE),
    r1_eag_m50bis55    = sum(r1_eag_m50bis55 * weight_tdw, na.rm = TRUE),
    r1_eag_m55bis60    = sum(r1_eag_m55bis60 * weight_tdw, na.rm = TRUE),
    r1_eag_m60bis65    = sum(r1_eag_m55bis60 * weight_tdw, na.rm = TRUE),
    r1_eag_m65bis75    = sum(r1_eag_m55bis60 * weight_tdw, na.rm = TRUE),
    # Female population
    r1_eag_w00bis03    = sum(r1_eag_w00bis03 * weight_tdw, na.rm = TRUE),
    r1_eag_w03bis06    = sum(r1_eag_w03bis06 * weight_tdw, na.rm = TRUE),
    r1_eag_w06bis10    = sum(r1_eag_w06bis10 * weight_tdw, na.rm = TRUE),
    r1_eag_w10bis15    = sum(r1_eag_w10bis15 * weight_tdw, na.rm = TRUE),
    r1_eag_w15bis18    = sum(r1_eag_w15bis18 * weight_tdw, na.rm = TRUE),
    r1_eag_w18bis20    = sum(r1_eag_w18bis20 * weight_tdw, na.rm = TRUE),
    r1_eag_w20bis25    = sum(r1_eag_w20bis25 * weight_tdw, na.rm = TRUE),
    r1_eag_w25bis30    = sum(r1_eag_w25bis30 * weight_tdw, na.rm = TRUE),
    r1_eag_w30bis35    = sum(r1_eag_w30bis35 * weight_tdw, na.rm = TRUE),
    r1_eag_w35bis40    = sum(r1_eag_w35bis40 * weight_tdw, na.rm = TRUE),
    r1_eag_w40bis45    = sum(r1_eag_w40bis45 * weight_tdw, na.rm = TRUE),
    r1_eag_w45bis50    = sum(r1_eag_w45bis50 * weight_tdw, na.rm = TRUE),
    r1_eag_w50bis55    = sum(r1_eag_w50bis55 * weight_tdw, na.rm = TRUE),
    r1_eag_w55bis60    = sum(r1_eag_w55bis60 * weight_tdw, na.rm = TRUE),
    r1_eag_w60bis65    = sum(r1_eag_w55bis60 * weight_tdw, na.rm = TRUE),
    r1_eag_w65bis75    = sum(r1_eag_w55bis60 * weight_tdw, na.rm = TRUE),
...
weighted_avg <- crosswalk |>
  mutate(weight = weights * r1_mba_a_haushalt) |>  # weighting base (area * HH)
  group_by(year, target_id) |>
  summarise(
...
housing_estimates_panel |>
  ...
  summarise(
    mean(r1_kkr_w_summe),
    mean(r1_mso_p_paare, na.rm = TRUE),
    sum(r1_ewa_a_gesamt, na.rm = TRUE)
  )

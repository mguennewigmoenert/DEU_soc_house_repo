...
#Preparing the data 
##############################################################################################################
# upload full data frame
data = data.frame(fread("/Users/maxmonert/Library/CloudStorage/Dropbox/Projects/Housing Project/data/temp/scm_prep_max.csv"))

# Define a function to create a three-letter ID: must 
generate_three_letter_id <- function(n) {
  # Generate a string using letters and format it as four letters
  paste0(LETTERS[(n - 1) %% 26 + 1], 
         LETTERS[((n - 1) %/% 26) %% 26 + 1], 
         LETTERS[((n - 1) %/% 676) %% 26 + 1])
}

# generate string ID, because numeric does not work with multisynth
data <- data %>%
  group_by(PLR_ID) %>%
  mutate(str_id = generate_three_letter_id(cur_group_id())) %>%
  ungroup()

# reset dynamic treatment for t
data$dynamic_treat_1st_m1 = data$dynamic_treat_1st
data$dynamic_treat_1st_m1 = ifelse(is.na(data$dynamic_treat_1st_m1) & !is.na(data$ty_treat0), 0, data$dynamic_treat_1st_m1)
data$dynamic_treat_1st_m1 = ifelse(data$ty_treat0==0, 1, data$dynamic_treat_1st_m1)

# reset dynamic treatment for t
data$dynamic_treat_1st_max_m1 = data$dynamic_treat_1st_max
data$dynamic_treat_1st_max_m1 = ifelse(is.na(data$dynamic_treat_1st_max_m1) & !is.na(data$ty_treat2), 0, data$dynamic_treat_1st_max_m1)
data$dynamic_treat_1st_max_m1 = ifelse(data$ty_treat0==0, 1, data$dynamic_treat_1st_max_m1)

# check treatment m1
x = data[c("dynamic_treat_1st_max_m1", "dynamic_treat_1st", "ty_treat0", "treated", "control")]

# subset data for analysis period
data_analysis = data |>
  filter(jahr <2017 & jahr>2006 & !is.na(treated))


ppool_syn <- multisynth(qm_miete_kalt ~ dynamic_treat_1st_max_m1, str_id, jahr, 
                          data_analysis, n_leads = 5, n_lags = 5
)
...
# subset for within A100 treatment
data_analysis_auto = data_analysis |>
  filter(
          (is.na(auto_ring) & treated==1) 
         # (is.na(auto_ring) & treated==1) 
           #| #(auto_ring ==1 & control ==1)
          | (is.na(auto_ring) & control ==1)
         )

# check variable selection
z = data_analysis_auto[c("auto_ring", "treated", "control", "dynamic_treat_1st")]

# run pooled synthetic control
ppool_syn_auto <- multisynth(qm_miete_kalt ~ dynamic_treat_1st_max_m1, str_id, jahr, 
                             data_analysis_auto, n_leads = 6, n_lags = 5
)

# summarize results
ppool_syn_summ_auto <- summary(ppool_syn_auto)

plot(ppool_syn_summ_auto)
plot(ppool_syn_summ_auto, levels = "Average")

# subset for within Sbahn treatment
data_analysis_sbahn = data_analysis |>
  filter((sbahn_r==1 & treated==1) 
         | (sbahn_r==1 & control ==1))
c = data_analysis[c("sbahn_r", "treated", "control")]

# run synthetic control did
ppool_syn_sbahn <- multisynth(qm_miete_kalt ~ dynamic_treat_1st_m1, str_id, jahr, 
                          data_analysis_sbahn, n_leads=6, n_lags = 4)

# summarize results
ppool_syn_summ_sbahn <- summary(ppool_syn_sbahn)

# plot event study
plot(ppool_syn_summ_sbahn, levels = "Average")
...
# 
x = data[c("PLR_ID", "jahr", "change_share1", "fy_treat1")] |>
  filter(PLR_ID == 1100103| PLR_ID == 1100104 | PLR_ID == 1200627)

# get list with all treated PLR's
# Obtain a list of all unique group IDs
treat_id <- data %>%
              filter(treated==1 & fy_treat2 < 2016
                     & is.na(auto_ring) # adjust for within or outside A100; not either 1 or NA
                     ) %>%
                select(PLR_ID) %>%     # Select the group_id column
                  distinct() %>%           # Keep only distinct (unique) group IDs
                    pull(PLR_ID)         # Extract the group_id column as a vector
...
treat_year <- data %>%
                filter(treated==1 & fy_treat2 < 2016
                  ...
                  group_by(PLR_ID) %>%                 # Group by group_id
                    summarize(fy_treat = unique(fy_treat2)) %>%  # Extract unique values as a list
                      ungroup() %>% 
                        dplyr::select(fy_treat)

# Convert to a list (optional, if needed as a list)
treat_year = unlist(c(treat_year))

# obtain all plr_id's within a100 and sbahn ring

s <- data.frame(treat_id, treat_year)
s$case <- paste(as.character(s$treat_id), as.character(s$treat_year), sep=".")
s$final <- paste0("_", as.character(s$case))

for (k in 1:length(s$treat_id)) {
  print(paste("Current PLR", s[[1]][k]))
  #k <- 32
  options(warn=0)
  Oldc = s[[1]][k]  
  Trea = s[[1]][k]
  Year = s[[2]][k] # treatment year
  Case = s$case[k]
  Set = s$final[k]
  control = s$control[k]
  ...
  data <- data[which(data$jahr >=2007 & data$jahr <=2016),]
  taker <- data %>% dplyr::filter(data$PLR_ID == Oldc)
  ...
  donors <- data %>%  dplyr::filter(control == 1 
                                    & is.na(auto_ring) # adjust for within or outside A100; not either 1 or NA
                                    )
  ...
  data <- data %>% group_by(PLR_ID) %>% dplyr::filter(all(!is.na(qm_miete_kalt) | PLR_ID==Oldc ))
  # Subset of all variables in treatment year
  tysub <- data[data$jahr == Year,]
  
  data$ln_qm_miete_kalt = log(data$qm_miete_kalt)

  # Only keep certain variables 
  tysub <- dplyr::select(tysub, PLR_ID, qm_miete_kalt)
  # tysub <- dplyr::select(tysub, PLR_ID, ln_qm_miete_kalt)
  
  # Rename variable 
  names(tysub)[names(tysub) == 'qm_miete_kalt'] <- 'iqm_miete_kalt'
  # names(tysub)[names(tysub) == 'ln_qm_miete_kalt'] <- 'iln_qm_miete_kalt'
  ...
  # data <- data %>% group_by(PLR_ID) %>% dplyr::mutate(d = qm_miete_kalt - iqm_miete_kalt, t = jahr - Year + 4)
  data <- data %>% group_by(PLR_ID) %>% dplyr::mutate(d = qm_miete_kalt - iqm_miete_kalt, ti = jahr - Year) |>
  ...
  #period.post <- (5:8)
  period.pre  <- which(unique(data$ti)<=0)-1
  period.post <- which(unique(data$ti)>0)-1
    # Treatment Indicator 
  #data <- data  %>% dplyr::mutate(treat = ifelse(plr_code == 5200526, 1,0))
  # Implement the Synthetic Control command 
  df  <- scdata(df = data,  features = features, constant = constant, cov.adj = cov.adj,  cointegrated.data = cointegrated.data, id.var = "PLR_ID" ,  
                time.var = "t", outcome.var = "d"
                , period.pre = period.pre
                , period.post = period.post
                , unit.tr = Trea, unit.co = setdiff(unique(data$PLR_ID),Trea))
  ...
  result <-  scpi(data = df, u.order = u.order, u.lags = u.lags, u.missp = u.missp,  e.order = e.order, e.lags = e.lags, w.constr = w.constr,
                  u.alpha = u.alpha, e.alpha = e.alpha, rho = rho,  rho.max = rho.max, sims = sims, cores = cores, e.method = e.method)
  ...
  yfit    <- data.frame(t = 1:length(unique(data$t)), ti = unique(data$ti), yfit = c(y.fit))
  # Actual observations 
  y.act <- rbind(result$data$Y.pre, result$data$Y.post)
  f <- c(period.pre, period.post)
  # Combine all the observations into one vector 
  yact    <- data.frame(t = 1:length(unique(data$t)), yact = c(y.act), case = Case)
  ...
  cis <- data.frame(t = c(period.post + 1), sclinsample = c(scl.insample), scrinsample = c(scr.insample),  sclgauss = c(scl.gauss), scrgauss = c(scr.gauss))
...
ggplot(dataforplot_all) + 
  ...
  geom_line(aes(x=ti, y=(yfit_all-yact_all)*(-1), colour = "Doppelganger gap (avg.)", size="Doppelganger gap (avg.)")) +
  ...
  scale_x_continuous(breaks=seq(-6, 5,1), expand=c(0.02,0.02)) +
  scale_y_continuous(
    # limits = c(-0.30, 0.10)
    #, breaks = c(-0.30,-0.25,-0.2,-0.15,-0.1,-0.05,0,0.05,0.1)
    #, labels = format(c("-30%","-25%","-20%","-15%","-10%","-5%","0%","+5%","+10%"),nsmall=1)
    #, expand=c(0.02,0.02)
  ) + 
  labs(title = "1st Change social housing - outside A100", x = "", y = "")+  theme_bw()+  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
...
ggsave("/Users/maxmonert/Desktop/Research/Projects/SocialHousing/output/max/scm/scm_out_a100_did.png", width = 6, height = 4, dpi = 300)
...
ggplot(data = dataforplot_all) +geom_ribbon(aes(ymin=sclgauss_all, ymax=scrgauss_all, x=ti, fill="90% CI (out-of-sample uncertainty)"), alpha = 1) +
  ...
  theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  scale_x_continuous(breaks=c(seq(-6,5,1)), expand=c(0.02,0.02))+
...
ggsave("/Users/maxmonert/Desktop/Research/Projects/SocialHousing/output/max/scm/scm_out_a100_seperate.png", width = 6, height = 4, dpi = 300)

# Generate the General Plots 
ggplot(data = dataforplot_all) + 
  geom_line(aes(y=yfit_all, x=ti, colour = "Untreated avg.", linetype= "Untreated avg.", size= "Untreated avg."))+
...
# ggsave("/Users/maxmonert/Desktop/Research/Projects/SocialHousing/output/es_max/scm/scm_parellel_all.png", width = 6, height = 4, dpi = 300)
